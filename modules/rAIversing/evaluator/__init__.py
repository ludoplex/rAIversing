...
from rAIversing.utils import load_func_data_from_json, save_to_json
...
def eval_p2im_firmwares(ai_module, parallel=1):
    usable_binaries = os.listdir(
        f"{BINARIES_ROOT}/p2im/stripped")  # ["Heat_Press", "CNC", "Gateway"]
    ...
    for binary in usable_binaries:
        ...
        if raie.is_import_needed():
            import_changes_to_existing_project(
                project_location=f"{PROJECTS_ROOT}/Evaluation",
                project_name="Evaluation",
                binary_name=f"{binary}_no_propagation")

    include_all = False

    result_table = Table(
        Column(header="Binary", style="bold bright_yellow on grey23"),
        Column(header="Actual\nAll", style="bold cyan1 on grey23",
               justify="center"),
        Column(header="Actual\nHigher", style="bold cyan2 on grey23",
               justify="center"),
        Column(header="Actual\nLowest", style="bold cyan3 on grey23",
               justify="center"),
        Column(header="Best\nCase",
               style="bold green on grey23", justify="center"),
        Column(header=" Worst\nCase",
        ...
        title="Evaluation Results", title_style="bold bright_red on grey23 ",
        style="on grey23", border_style="bold bright_green",
    ...
    for binary in usable_binaries:
        ...
        funcs_actual, layers_actual = load_func_data_from_json(
            f"Evaluation/{binary}.json")
        funcs_orig, layers_orig = load_func_data_from_json(
            f"Evaluation/{binary}_original.json")
        funcs_gt, layers_gt = load_func_data_from_json(
            f"Evaluation/{binary}_original_stripped.json")
        funcs_no_prop, layers_no_prop = load_func_data_from_json(
            f"Evaluation/{binary}_no_propagation.json")

        sum_actual, counted_actual = run_comparison(
            include_all, funcs_orig, funcs_actual,
            dc_dict, eval_dict)
        score_actual = sum_actual / counted_actual

        sum_hfl, counted_hfl = run_comparison(
            include_all, funcs_orig, funcs_actual,
            dc_dict_hfl, eval_dict_hfl, skip_lfl=True)
        score_hfl = sum_hfl / counted_hfl

        sum_lfl, counted_lfl = run_comparison(
            include_all, funcs_orig, funcs_actual,
            skip_hfl=True)
        ...
        save_to_json(eval_dict, f"Evaluation/{binary}_evaluation.json")
        save_to_json(dc_dict_GT,
                     f"Evaluation/{binary}_GT_comparison.json")
        save_to_json(eval_dict_GT,
                     f"Evaluation/{binary}_GT_evaluation.json")
        save_to_json(dc_dict_no_prop,
...
def run_comparison(include_all, original_functions, reversed_functions,
    dc_dict=None, evaluation_dict=None, skip_lfl=False, skip_hfl=False):
    overall_score = 0
    ...
    for orig_fun_name, orig_data in original_functions.items():
        ...
        score = compute_similarity_score(orig_fun_name, reversed_name,
                                         entrypoint)
        if wrt_to_dicts:
            dc_dict[orig_fun_name] = reversed_name
            evaluation_dict[entrypoint] = {
                orig_fun_name: reversed_name,
                "score": score
            }
