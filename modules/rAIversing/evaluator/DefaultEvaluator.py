            ...
            for run in range(1, self.runs + 1):
                for group, layer in self.results[model][source_dir][run][binary][comp_type].items():
                    if group not in self.results[model][source_dir][0][binary][comp_type].keys():
                        self.results[model][source_dir][0][binary][comp_type][group] = {"score": 0, "count": 0}
                    self.results[model][source_dir][0][binary][comp_type][group]["score"] += layer["score"]
                    self.results[model][source_dir][0][binary][comp_type][group]["count"] += layer["count"]

            for group, layer in self.results[model][source_dir][0][binary][comp_type].items():
                self.results[model][source_dir][0][binary][comp_type][group]["score"] /= self.runs
                self.results[model][source_dir][0][binary][comp_type][group]["count"] /= self.runs

    def get_cumulative_results(self, model, source_dir, binary):
        return self.results[model][source_dir][0][binary]
