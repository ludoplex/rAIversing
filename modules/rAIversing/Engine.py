...
class rAIverseEngine:
  ...
  def get_sorted_missing(self):
    ...
    sorted_missing = sorted(missing,
                            key=lambda k: len(list(
                              set(self.functions[k]['called']).intersection(
                                missing))),
                            reverse=False)
    sorted_missing = sorted(sorted_missing,
                            key=lambda k: len(list(
                              set(self.functions[k]['calling']).intersection(
                                missing))),
                            reverse=True)
  ...
  def undo_bad_renaming(self, renaming_dict, code, original_code):
    ...
    renaming_dict_sorted = dict(
      sorted(renaming_dict.items(), key=lambda item: (-len(item[1]), item[1])))
    # print("")
  ...
  def handle_result_processing(self, name, result):
    try:
      ...
      improved_code = self.undo_bad_renaming(renaming_dict, improved_code,
                                             to_be_improved_code)
      improved_code = check_and_fix_double_function_renaming(improved_code,
                                                             renaming_dict,
                                                             name)
    ...
    for name, data in self.functions.items():
      ...
      if name == current_name:
        ...
        if not current_name.endswith(address_postfix):
          ...
          self.functions[name]["current_name"] = new_name
          self.functions[name]["code"] = data["code"].replace(current_name,
                                                              new_name)
  ...
  def skip_do_nothing(self):
    renaming_dict = {}
    for name, data in self.functions.items():
      if check_do_nothing(data["code"]) and not data["improved"] and not data[
        "skipped"]:
